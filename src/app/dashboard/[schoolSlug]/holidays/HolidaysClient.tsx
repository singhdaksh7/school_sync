...
export default function HolidaysClient({ initialHolidays, schoolId }: Props) {
  ...
  async function save() {
    ...
    setDialogOpen(false); setForm({ name: "", date: "" });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{upcoming.length > 0 ? `${upcoming.length} upcoming holiday${upcoming.length > 1 ? "s" : ""}` : "No upcoming holidays"}</p>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {upcoming.slice(0, 5).map((h) => (
              <span key={h.id} className="inline-flex items-center gap-1.5 text-xs bg-white border border-amber-200 text-amber-800 px-2.5 py-1 rounded-full">
                <CalendarOff className="w-3 h-3" />{h.name} — {format(new Date(h.date), "dd MMM")}
              </span>
            ))}
            {upcoming.length > 5 && <span className="text-xs text-amber-600">+{upcoming.length - 5} more</span>}
          </div>
        </div>
      )}

      {loading ? (
        <div className="space-y-2">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-14 bg-gray-100 animate-pulse rounded-lg" />)}</div>
      ) : holidays.length === 0 ? (
        <Card><CardContent className="py-20 text-center"><CalendarOff className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-500 font-medium">No holidays added yet</p></CardContent></Card>
      ) : (
        <div className="space-y-6">
          {Object.values(grouped).map((group) => (
            <div key={group.label}>
              ...
              <div className="space-y-2">
                {group.items.map((h) => {
                  const d = new Date(h.date); const isPast = d < today;
                  return (
                    <div key={h.id} className={`flex items-center justify-between px-4 py-3 rounded-lg border ${isPast ? "bg-gray-50 border-gray-100" : "bg-white border-gray-200"}`}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5"><Label>Holiday Name *</Label><Input placeholder="e.g. Diwali, Republic Day" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
            <div className="space-y-1.5">
              <Label>Date *</Label>
              <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full h-10 px-3 rounded-md border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <DialogFooter>
