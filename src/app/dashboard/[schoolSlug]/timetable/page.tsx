...
export default function TimetablePage() {
  ...
  const [allBusySlots, setAllBusySlots] = useState<BusySlot[]>([]);
  ...
  // dayPeriods: maps day index (1=Mon…6=Sat) → comma-separated period string e.g. "1,3"
  ...
  useEffect(() => {
    fetch(`/api/school-by-slug/${schoolSlug}`).then((r) => r.json()).then((d) => {
      setSchoolId(d.id);
      Promise.all([
        fetch(`/api/schools/${d.id}/classes`).then((r) => r.json()),
        fetch(`/api/schools/${d.id}/teachers`).then((r) => r.json()),
        fetch(`/api/schools/${d.id}/timetable?allSlots=true`).then((r) => r.json()),
      ]).then(([cls, tch, all]) => {
        setClasses(cls);
        setTeachers(tch);
        setAllBusySlots(all.slots || []);
      });
    });
  }, [schoolSlug]);

  const fetchSlots = useCallback(async (sid: string, sectionId: string) => {
    setLoading(true);
    const res = await fetch(`/api/schools/${sid}/timetable?sectionId=${sectionId}`);
  ...
  useEffect(() => {
    if (schoolId && selectedSectionId) fetchSlots(schoolId, selectedSectionId);
    else setSlots([]);
  }, [schoolId, selectedSectionId, fetchSlots]);
  ...
  function onDrop(e: React.DragEvent, day: number, period: number) {
    ...
    // Pre-fill dropped slot's day with that period
  ...
  // Parse dayPeriods into list of { day, period } — returns null if any value is invalid
  ...
  async function confirmAssign() {
    ...
    if (slotsToAssign === null) {
      setValidationError(`All period numbers must be between 1 and ${periodsPerDay}.`);
      return;
    }
    if (slotsToAssign.length === 0) {
      setValidationError("Enter at least one period for any day.");
      return;
    }

    // Check conflicts (only on first attempt — if conflicts already shown, skip check and proceed)
    if (conflicts.length === 0) {
      const found: ConflictEntry[] = [];
      await Promise.all(
        slotsToAssign.map(async ({ day, period }) => {
          try {
            const res = await fetch(
              `/api/schools/${schoolId}/timetable?checkConflict=true&teacherId=${pendingTeacherId}&dayOfWeek=${day}&period=${period}&sectionId=${selectedSectionId}`
            );
            const data = await res.json();
            if (data.conflict) {
              found.push({ day, period, className: data.className, sectionName: data.sectionName, subject: data.subject });
            }
          } catch { /* non-blocking */ }
        })
      );
      if (found.length > 0) {
        setConflicts(found);
        return; // show conflicts — user must click again to confirm
      }
    }
    ...
    setSaving(false);
    // Refresh all busy slots for free-period display
    fetch(`/api/schools/${schoolId}/timetable?allSlots=true`)
      .then((r) => r.json())
      .then((d) => setAllBusySlots(d.slots || []));
    closeDialog();
  }
  ...
  // Free periods for a teacher on a given day
  function getFreePeriods(teacherId: string, day: number): number[] {
    const busy = new Set(
      allBusySlots.filter((s) => s.teacherId === teacherId && s.dayOfWeek === day).map((s) => s.period)
    );
    return Array.from({ length: periodsPerDay }, (_, i) => i + 1).filter((p) => !busy.has(p));
  }

  // Compact free-period label: "M:3,5 W:1,4" — only days with at least one free period shown
  ...
  async function savePeriodsPerDay() {
    ...
    setPeriodsPerDay(n);
    if (selectedSectionId) fetchSlots(schoolId, selectedSectionId);
  }
          ...
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="space-y-1.5 ml-auto">
              <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide flex items-center gap-1">
                <Settings2 className="w-3 h-3" /> Periods/Day
              </Label>
      ...
      {!selectedSectionId ? (
        <Card>
          <CardContent className="py-20 text-center">
            ...
            <p className="text-gray-500 font-medium">Select a class and section to view the timetable</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr>
                    ...
                    {DAYS.map((d) => (
                      <th key={d} className="px-2 py-2 text-xs font-semibold text-gray-700 bg-gray-50 border border-gray-200 text-center min-w-[130px]">{d}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                            ...
                            {slot?.teacher ? (
                              <div className="relative group bg-blue-50 border border-blue-200 rounded-md px-2 py-1.5 min-h-[52px]">
                                ...
                                <button
                                  ...
                                  className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 text-blue-400 hover:text-red-500 transition-opacity"
                                >
                                  <X className="w-3 h-3" />
                                </button>
                              </div>
                              ...
                              <div
                                className={cn(
                                  "min-h-[52px] rounded-md border-2 border-dashed flex items-center justify-center text-gray-300 transition-colors",
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-1">
            {pendingTeacher && (
              <p className="text-sm text-gray-600">
                Teacher: <strong>{pendingTeacher.name}</strong>
                {pendingTeacher.subject && <span className="text-gray-400"> · {pendingTeacher.subject}</span>}
              </p>
            )}
                  ...
                  return (
                    <div key={d} className="flex items-center gap-3">
                      ...
                      <Input
                        ...
                        value={val}
                        onChange={(e) => {
                          setDayPeriods((prev) => ({ ...prev, [d]: e.target.value }));
                          setConflicts([]);
                          setValidationError("");
                        }}
                        className={cn(
                      ...
                      />
                      {invalid && (
                        <span className="text-xs text-red-500 shrink-0">1–{periodsPerDay}</span>
                      )}
                    </div>
                  );
                })}
              ...
              <div className="bg-amber-50 border border-amber-300 rounded-lg px-3 py-3 space-y-1.5">
                ...
                {conflicts.map((c, i) => (
                  <p key={i} className="text-xs text-amber-800 pl-6">
                    {DAYS[c.day - 1]} P{c.period} — already assigned to Class {c.className} – Sec {c.sectionName}
                    {c.subject ? ` (${c.subject})` : ""}
                  </p>
                ))}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={saving}
              className={hasConflicts ? "bg-amber-600 hover:bg-amber-700" : ""}
            >
