...
export async function DELETE(
  _req: Request,
  { params }: { params: Promise<{ schoolId: string; holidayId: string }> }
) {
  const { schoolId, holidayId } = await params;
  const session = await auth();
  if (!session?.user?.id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!(await canAccess(schoolId, session.user.id))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  await prisma.holiday.deleteMany({ where: { id: holidayId, schoolId } });
