'use client'
// =============================================================================
// SchoolSync — #1 White-label login screen.
// Shows the SaaS hero, school selector (drives the white-label theme),
// and role sign-in cards. Fully self-contained; receives only an onEnter callback.
// =============================================================================

import { motion } from 'framer-motion'
import { Card } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { GraduationCap, Users, BookOpen, Sparkles, Shield, Building2, ArrowRight, CheckCircle2, type LucideIcon } from 'lucide-react'
import { useSchool, SchoolLogo } from './theme'
import type { RoleId } from '@/lib/schoolsync/types'

interface RoleCard { id: RoleId; label: string; icon: LucideIcon; desc: string; color: string }
...
export interface LandingProps {
  /** Called when a user picks a role to sign in as. Wire this to your real auth flow. */
      ...
      <main className="max-w-7xl mx-auto px-6 pt-10 pb-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-5xl md:text-6xl font-bold tracking-tight leading-[1.05]">The modern OS for schools.</h1>
            <p className="text-lg text-muted-foreground mt-5 max-w-lg">
              SchoolSync brings admins, teachers, parents, and students together with white-label branding,
              real-time analytics, and beautiful UX across web &amp; mobile.
            </p>
              ...
              <div className="grid grid-cols-3 gap-2 mt-3">
                {schools?.map(s => (
                  <button
                    key={s.id}
                    onClick={() => setSchool(s)}
                    className={`p-3 rounded-xl border text-left transition-all ${school?.id === s.id ? 'border-primary ring-2 ring-primary/20 bg-primary/5' : 'border-border hover:border-foreground/20'}`}
                  >
                ...
                {ROLE_CARDS.map(r => {
                  ...
                  return (
                    <button
                      ...
                      className="group p-4 rounded-xl border border-border hover:border-foreground/20 hover:shadow-md transition-all text-left bg-card"
                    >
                      ...
                      <div className="mt-2 text-xs text-primary opacity-0 group-hover:opacity-100 transition flex items-center gap-1">
