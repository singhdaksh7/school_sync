  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
        <div
          className={cn("h-full rounded-full transition-all", pct > 80 ? "bg-red-400" : pct > 50 ? "bg-orange-400" : "bg-blue-400")}
          style={{ width: `${pct}%` }}
        />
      </div>
...
export default function TeacherWorkloadPage() {
  ...
  useEffect(() => {
    fetch(`/api/school-by-slug/${schoolSlug}`).then((r) => r.json()).then((d) => {
      setSchoolId(d.id);
      fetch(`/api/schools/${d.id}/teachers/workload`).then((r) => r.json()).then((w) => {
        setWorkload(w);
        setLoading(false);
      });
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            <p className="text-xs text-gray-500">Avg Periods/Week</p>
            <p className="text-2xl font-bold text-gray-900">{avgPeriods}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center gap-2">
              <AlertTriangle className={cn("w-4 h-4", overloaded.length > 0 ? "text-orange-500" : "text-gray-300")} />
              <div>
                <p className="text-xs text-gray-500">Overloaded</p>
                <p className="text-2xl font-bold text-gray-900">{overloaded.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center gap-2">
              <TrendingDown className={cn("w-4 h-4", underloaded.length > 0 ? "text-blue-400" : "text-gray-300")} />
              <div>
                <p className="text-xs text-gray-500">No Timetable</p>
                <p className="text-2xl font-bold text-gray-900">{underloaded.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {loading ? (
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">All Teachers — Sorted by Load</CardTitle>
          </CardHeader>
                ...
                <div key={t.id} className="py-4 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {t.periodsPerWeek > avgPeriods * 1.5 && (
                        <Badge variant="outline" className="text-xs bg-orange-50 text-orange-600 border-orange-200">Overloaded</Badge>
                      )}
                      {t.periodsPerWeek === 0 && (
                        <Badge variant="outline" className="text-xs bg-gray-50 text-gray-400 border-gray-200">No timetable</Badge>
                      )}
                    </div>
