...
interface SchoolData { id: string; name: string; address: string | null; phone: string | null; email: string | null; website: string | null; slug: string; admins: { id: string; name: string; email: string }[] }
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch(`/api/school-by-slug/${schoolSlug}`).then((r) => r.json()).then((d) => {
      setSchoolId(d.id);
      fetch(`/api/schools/${d.id}`).then((r) => r.json()).then((s) => {
        setSchool(s);
        setForm({ name: s.name, address: s.address || "", phone: s.phone || "", email: s.email || "", website: s.website || "" });
      });
    });
  }, [schoolSlug]);
  ...
  if (!school) return <div className="text-center py-20 text-gray-400">Loading...</div>;
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input className="pl-9" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label>Address</Label>
              <div className="relative">
                ...
                <Input className="pl-9" placeholder="School address" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input className="pl-9" type="email" placeholder="info@school.edu" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
                </div>
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input className="pl-9" placeholder="https://school.edu" value={form.website} onChange={(e) => setForm({ ...form, website: e.target.value })} />
              </div>
            </div>
            <div className="space-y-1.5">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Users className="w-4 h-4" /> Admin Members
            </CardTitle>
          </CardHeader>
          <CardContent>
