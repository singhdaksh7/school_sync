...
export default function ExamSchemesPage() {
  ...
  async function fetchSchemes(sid: string) {
    setLoading(true);
    const res = await fetch(`/api/schools/${sid}/exam-schemes`);
  ...
  function removeExamRow(i: number) {
  ...
  async function save() {
    ...
    const parsedExams = exams.map((e, i) => ({
      name: e.name.trim(),
      maxMarks: parseInt(e.maxMarks, 10),
      order: i,
    }));
    if (parsedExams.some((e) => !e.name || isNaN(e.maxMarks) || e.maxMarks < 1)) {
      setError("All exams must have a name and valid marks");
      return;
    }
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-20 text-center">
            ...
            <p className="text-gray-500 font-medium">No exam schemes yet</p>
            <p className="text-gray-400 text-sm mt-1">Create a scheme to define your exam structure</p>
          </CardContent>
        </Card>
      ) : (
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2 max-h-[65vh] overflow-y-auto pr-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Annual Exam 2024-25"
                value={schemeName}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label>Exams</Label>
                <Button type="button" variant="outline" size="sm" onClick={addExamRow} className="gap-1 h-7 text-xs">
              ...
              {exams.map((exam, i) => (
                <div key={i} className="flex gap-2 items-center">
                  ...
                  <Input
                    ...
                    min={1}
                    value={exam.maxMarks}
                    onChange={(e) => updateExam(i, "maxMarks", e.target.value)}
                    className="w-20 text-center"
                  />
                  <span className="text-xs text-gray-400 shrink-0">marks</span>
                  {exams.length > 1 && (
                    <button onClick={() => removeExamRow(i)} className="text-gray-300 hover:text-red-500">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
