...
interface Arrangement {
  id: string; date: string; period: number; subject: string | null;
  substituteTeacher: { name: string } | null;
  section: { name: string; class: { name: string } };
}

interface LeaveRequest {
  id: string; type: "STUDENT" | "TEACHER"; reason: string;
  fromDate: string; toDate: string; status: "PENDING" | "APPROVED" | "REJECTED";
...
interface Props { initialLeaves: LeaveRequest[]; schoolId: string }

export default function LeavesClient({ initialLeaves, schoolId }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">{pendingCount > 0 ? `${pendingCount} pending request${pendingCount > 1 ? "s" : ""}` : "No pending requests — teachers submit requests from their portal"}</p>
      </div>
      ...
      <div className="flex border-b border-gray-200">
        {(["PENDING", "APPROVED", "REJECTED"] as StatusTab[]).map((s) => {
          ...
          return (
            <button key={s} onClick={() => setStatusTab(s)} className={cn(
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 bg-gray-100 animate-pulse rounded-xl" />)}</div>
      ) : filtered.length === 0 ? (
        <Card><CardContent className="py-16 text-center"><ClipboardList className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-500 font-medium">No {statusTab.toLowerCase()} leave requests</p></CardContent></Card>
      ) : (
          ...
          {filtered.map((l) => {
            ...
            return (
              <Card key={l.id} className="hover:shadow-sm transition-shadow">
                <CardContent className="pt-4 pb-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Badge variant="outline" className={cn("text-xs", STATUS_CONFIG[l.status].color)}>{STATUS_CONFIG[l.status].label}</Badge>
                      {l.status === "PENDING" && (
                        <>
                          <Button size="icon" variant="ghost" className="h-7 w-7 text-green-600 hover:bg-green-50" title="Approve" onClick={() => updateStatus(l.id, "APPROVED")}><Check className="w-3.5 h-3.5" /></Button>
                          <Button size="icon" variant="ghost" className="h-7 w-7 text-red-500 hover:bg-red-50" title="Reject" onClick={() => updateStatus(l.id, "REJECTED")}><X className="w-3.5 h-3.5" /></Button>
                        </>
                      )}
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-gray-400 hover:text-red-500" onClick={() => deleteLeave(l.id)}><Trash2 className="w-3.5 h-3.5" /></Button>
                    </div>
                  </div>
                </CardContent>
                ...
                {l.status === "APPROVED" && (
                  <>
                    <button onClick={() => fetchArrangements(l.id)} className="w-full flex items-center justify-between px-5 py-2 border-t border-gray-100 text-xs text-blue-600 hover:bg-blue-50 transition-colors">
                      <span className="flex items-center gap-1.5"><CalendarDays className="w-3.5 h-3.5" />View Arrangements</span>
                      ...
                      <div className="px-5 pb-4 border-t border-gray-100 bg-gray-50">
                        {!arrangements[l.id] ? (
                          ...
                          <div className="space-y-1.5 pt-3">
                            {arrangements[l.id].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime() || a.period - b.period).map((arr) => (
                              <div key={arr.id} className="flex items-center justify-between text-xs bg-white rounded-lg px-3 py-2 border border-gray-200">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-gray-500">{arr.section.class.name}-{arr.section.name}{arr.subject ? ` · ${arr.subject}` : ""}</span>
                                </div>
                                {arr.substituteTeacher ? <span className="text-green-700 font-medium">{arr.substituteTeacher.name}</span> : <span className="text-orange-500 font-medium">No substitute found</span>}
                              </div>
                            ))}
                          </div>
                        )}
