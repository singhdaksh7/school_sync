...
interface SummaryEntry {
  id: string; name: string; rollNo?: string; subject?: string; sectionLabel?: string;
...
interface Props { initialSections: Section[]; schoolId: string }

export default function ReportsClient({ initialSections, schoolId }: Props) {
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="pt-5 pb-5">
          <div className="flex flex-wrap gap-4 items-end">
            <div className="space-y-1.5">
              ...
              <div className="flex rounded-lg border border-gray-200 overflow-hidden">
                {(["STUDENT", "TEACHER"] as const).map((m) => (
                  <button key={m} onClick={() => { setMode(m); setSummary([]); }}
            ...
            <Button onClick={fetchReport} disabled={loading} className="gap-2">
              <BarChart2 className="w-4 h-4" />{loading ? "Loading..." : "Generate Report"}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {summary.length > 0 && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card><CardContent className="pt-5 pb-5"><p className="text-sm text-gray-500 font-medium">Average Attendance</p><p className="text-3xl font-bold text-gray-900 mt-1">{avgPct}%</p><p className="text-xs text-gray-400 mt-1">{totalDays} days in range</p></CardContent></Card>
            <Card><CardContent className="pt-5 pb-5"><p className="text-sm text-gray-500 font-medium">Total {mode === "STUDENT" ? "Students" : "Teachers"}</p><p className="text-3xl font-bold text-gray-900 mt-1">{summary.length}</p><p className="text-xs text-gray-400 mt-1">with at least one record</p></CardContent></Card>
            <Card><CardContent className="pt-5 pb-5"><p className="text-sm text-gray-500 font-medium">Below 75%</p><p className={cn("text-3xl font-bold mt-1", belowThreshold > 0 ? "text-red-600" : "text-green-600")}>{belowThreshold}</p><p className="text-xs text-gray-400 mt-1">need attention</p></CardContent></Card>
          </div>
          ...
          <Card>
            <CardHeader className="pb-3"><CardTitle className="text-base">{mode === "STUDENT" ? "Student" : "Teacher"} Attendance — {from} to {to}</CardTitle></CardHeader>
                  ...
                  {sorted.map((entry) => {
                    ...
                    return (
                      <tr key={entry.id} className={cn("border-b border-gray-50 hover:bg-gray-50", isLow && "bg-red-50/30")}>
                        ...
                        <td className="py-2.5 px-3 text-center"><span className="font-semibold text-green-700">{entry.present}</span></td>
                        <td className="py-2.5 px-3 text-center"><span className={cn("font-semibold", entry.absent > 0 ? "text-red-600" : "text-gray-400")}>{entry.absent}</span></td>
                        <td className="py-2.5 px-3 text-center"><span className={cn("font-semibold", entry.late > 0 ? "text-yellow-600" : "text-gray-400")}>{entry.late}</span></td>
                        <td className="py-2.5 px-3 text-center text-gray-600">{entry.total}</td>
                        <td className="py-2.5 px-3 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div className={cn("h-full rounded-full", pct >= 75 ? "bg-green-500" : pct >= 50 ? "bg-yellow-500" : "bg-red-500")} style={{ width: `${pct}%` }} />
                            </div>
                            <Badge variant="secondary" className={cn("text-xs min-w-[3rem] justify-center", pct >= 75 ? "bg-green-100 text-green-700" : pct >= 50 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700")}>{pct}%</Badge>
                          </div>
                        </td>
                        <td className="py-2.5 px-3">{isLow && <Badge variant="destructive" className="text-xs">Low</Badge>}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {summary.length === 0 && !loading && (
        <Card><CardContent className="py-20 text-center"><BarChart2 className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-500 font-medium">No data yet</p><p className="text-gray-400 text-sm mt-1">Select filters and click Generate Report</p></CardContent></Card>
      )}
    </div>
  );
}
