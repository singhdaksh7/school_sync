...
type Tab = "catalogue" | "books" | "reservations";
...
type Book = { id: string; title: string; authors: string | null; availableCopies?: number };
...
export default function StudentLibraryPage() {
  ...
  const tabs: Tab[] = ["catalogue", "books", "reservations"];

  useEffect(() => {
    api(`/api/student/library/loans?limit=100`).then((r) => r.ok && setLoans(((r.data as Json).data as Loan[]) ?? []));
  }, []);
  const outstanding = loans.reduce((s, l) => s + Number(l.fineOutstanding || 0), 0);

  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-foreground">{t("library.title")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{t("library.student.outstanding")}: {outstanding.toFixed(2)}</p>
      </div>
      <div className="flex flex-wrap gap-2 border-b border-border">
        {tabs.map((tb) => (
          <button key={tb} onClick={() => setTab(tb)} className={`rounded-t-lg px-4 py-2 text-sm font-medium ${tab === tb ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}>
            {tb === "catalogue" ? t("library.student.catalogue") : tb === "books" ? t("library.student.myBooks") : t("library.student.reservations")}
          </button>
        ))}
      </div>
      {tab === "catalogue" && <StudentCatalogue />}
      {tab === "books" && <StudentLoans loans={loans} />}
      {tab === "reservations" && <StudentReservations />}
    </div>
  );
}
...
function StudentCatalogue() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const r = await api(`/api/student/library/catalogue?q=${encodeURIComponent(q)}&limit=50`);
  ...
  async function reserve(bookId: string) {
    const r = await api(`/api/student/library/reservations`, { method: "POST", body: JSON.stringify({ bookId }) });
...
function StudentLoans({ loans }: { loans: Loan[] }) {
  const { t } = useTranslation();
...
function StudentReservations() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const r = await api(`/api/student/library/reservations`);
  ...
  async function cancel(id: string) { const r = await api(`/api/student/library/reservations/${id}`, { method: "DELETE" }); if (r.ok) load(); }
