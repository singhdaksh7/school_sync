...
interface Props { initialLogs: AuditLog[]; schoolId: string }

export default function AuditLogsClient({ initialLogs, schoolId }: Props) {
  const [logs, setLogs] = useState<AuditLog[]>(initialLogs);
  const [loading, setLoading] = useState(false);
  const [filterAction, setFilterAction] = useState("");
  const [filterEntity, setFilterEntity] = useState("");

  const fetchLogs = useCallback(async (action: string, entity: string) => {
    setLoading(true);
  ...
  function handleActionChange(val: string) {
    setFilterAction(val);
    fetchLogs(val, filterEntity);
  }

  function handleEntityChange(val: string) {
    setFilterEntity(val);
    fetchLogs(filterAction, val);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-14 bg-gray-100 animate-pulse rounded-xl" />)}</div>
      ) : logs.length === 0 ? (
        <Card>
