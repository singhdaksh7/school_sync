...
export const TEACHER_OPERATIONS_CAPABILITIES = [
  ...
  // Attendance v2: reuses PERMISSION_CATALOG.ATTENDANCE ("VIEW"/"APPROVE_CORRECTION")
  // the same way TEACHER_LEAVE_* reuses PERMISSION_CATALOG.LEAVE above — one
  // "ATTENDANCE":"APPROVE_CORRECTION" check underlies both APPROVE and REJECT
  // (the PATCH endpoint's `action` distinguishes the resulting status).
  "ATTENDANCE_CORRECTION_VIEW",
  "ATTENDANCE_CORRECTION_APPROVE",
  "ATTENDANCE_CORRECTION_REJECT",
  "ATTENDANCE_EMERGENCY_CORRECTION",
  "ARRANGEMENTS_VIEW",
  "ARRANGEMENTS_MANAGE",
  "SUBSTITUTIONS_VIEW",
