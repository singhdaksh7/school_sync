...
import { certificateReviewStartSchema } from "@/lib/certificates/validation";
import { startCertificateReview } from "@/lib/certificates/actions";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ schoolId: string; requestId: string }> }) {
  ...
  const parsed = certificateReviewStartSchema.safeParse(body);
  if (!parsed.success) return NextResponse.json({ error: "Invalid input", details: parsed.error.flatten() }, { status: 400 });

  const result = await startCertificateReview({ schoolId, requestId, expectedVersion: parsed.data.version });
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: result.status });

  await logAudit({
    action: "CERTIFICATE_REQUEST_REVIEW_STARTED",
