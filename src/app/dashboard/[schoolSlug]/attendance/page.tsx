...
export default function AttendancePage() {
  ...
  async function fetchMeta(sid: string) {
    ...
    fetchStudents(sid, "all");
    fetchExisting(sid, new Date().toISOString().split("T")[0], "STUDENT", null);
  }

  const fetchStudents = useCallback(async (sid: string, sectionId: string) => {
    const url = sectionId !== "all"
      ? `/api/schools/${sid}/students?sectionId=${sectionId}`
      : `/api/schools/${sid}/students`;
    const res = await fetch(url);
    setStudents(await res.json());
  }, []);

  const fetchExisting = useCallback(async (sid: string, d: string, type: string, sectionId: string | null) => {
    setLoading(true);
    const url = `/api/schools/${sid}/attendance?date=${d}&type=${type}${sectionId && sectionId !== "all" ? `&sectionId=${sectionId}` : ""}`;
    const res = await fetch(url);
  ...
  useEffect(() => {
    if (!schoolId) return;
    if (mode === "STUDENT") {
      fetchStudents(schoolId, selectedSection);
      fetchExisting(schoolId, date, "STUDENT", selectedSection);
    } else {
      fetchExisting(schoolId, date, "TEACHER", null);
    }
  }, [mode, date, selectedSection, schoolId, fetchStudents, fetchExisting]);
