...
export default function ResultsPage() {
  ...
  useEffect(() => {
    fetch(`/api/school-by-slug/${schoolSlug}`).then((r) => r.json()).then((d) => {
      setSchoolId(d.id);
      Promise.all([
        fetch(`/api/schools/${d.id}/exam-schemes`).then((r) => r.json()),
        fetch(`/api/schools/${d.id}/classes`).then((r) => r.json()),
      ]).then(([schemesData, classesData]) => {
  ...
  const loadResults = useCallback(async (schemeId: string, sectionId: string) => {
    if (!schoolId || !schemeId || !sectionId) return;
    setLoading(true);
    ...
    data.forEach((r: ResultEntry & { exam: { id: string } }) => {
      ...
      map[r.studentId][r.examId] = r.marks;
    });
  ...
  // When exam is selected, prefill marks from existing
  useEffect(() => {
  ...
  // Stats for currently selected exam
  const enteredMarks = students
    .map((s) => parseFloat(marks[s.id] ?? ""))
    .filter((m) => !isNaN(m));
  ...
  // All-exams view: total marks per student
  const allExamsSummary = selectedScheme && selectedSection && students.length > 0
    ? students.map((s) => {
        const total = selectedScheme.exams.reduce((sum, e) => {
          const key = `${s.id}_${e.id}`;
          return sum + (existing[key] ?? 0);
        }, 0);
      ...
      {!selectedScheme || !selectedSection ? (
        <Card>
          <CardContent className="py-20 text-center">
            ...
            <p className="text-gray-500 font-medium">Select a scheme and section to view results</p>
          </CardContent>
        </Card>
      ) : loading ? (
        ...
        <>
          {/* Summary table across all exams */}
          {allExamsSummary.length > 0 && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-blue-600" />
                  Overall Summary — {selectedScheme.name}
                </CardTitle>
              </CardHeader>
                    ...
                    {allExamsSummary.map(({ student, total, maxTotal, filled }) => {
                      ...
                      return (
                        <tr key={student.id} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          {selectedScheme.exams.map((e) => {
                            const key = `${student.id}_${e.id}`;
                            const val = existing[key];
                            return (
                              <td key={e.id} className="py-2.5 px-3 text-center">
                                {val !== undefined ? (
                                  <span className={cn("font-medium", val >= e.maxMarks * 0.33 ? "text-gray-900" : "text-red-600")}>
                                    {val}
                                  </span>
                                ) : (
                          ...
                          <td className="py-2.5 px-3 text-center font-semibold text-gray-900">
                            {filled > 0 ? `${total}/${maxTotal}` : "—"}
                          </td>
                          <td className="py-2.5 px-3 text-center">
                            {filled > 0 ? (
                              <Badge
                                variant={pct >= 33 ? "default" : "destructive"}
                                className={cn("text-xs", pct >= 60 ? "bg-green-100 text-green-700 hover:bg-green-100" : pct >= 33 ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-100" : "")}
                              >
                                {pct}%
                              </Badge>
                            ) : "—"}
                      ...
                      <div key={student.id} className="flex items-center gap-4 py-2.5 px-4 rounded-lg border border-gray-100 hover:bg-gray-50">
                        ...
                        {existing[`${student.id}_${selectedExam.id}`] !== undefined && (
                          <Badge variant="secondary" className="text-xs">
                            saved: {existing[`${student.id}_${selectedExam.id}`]}
                          </Badge>
                        )}
          ...
          {students.length === 0 && (
            <Card>
              <CardContent className="py-16 text-center">
                <p className="text-gray-400">No students in this section.</p>
              </CardContent>
            </Card>
          )}
        </>
